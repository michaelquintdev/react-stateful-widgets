...
export default function Programmers() {
  ...
  const [featuredDev, setFeaturedDev] = useState(listOfAwesome);
  ...
  const getNameOfFeatured = () => {
    ...
    return featuredDev.filter(item => item.id === featuredId)
      [0].name;
  };
  ...
  const style = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: ((featuredId === '') ? 'royalblue' : 'gold'), // 🤔 color turns to gold, when celebrating
  };
      ...
      <div id='featured' style={style}>
        {
          ...
          ((featuredId === '') ? `Pick an awesome programmer` : `🎉 Let's celebrate 
          ${getNameOfFeatured()}! 🥳`)
